...
public class Clients {
    ...
    public Clients(int id, String nom, String prenom, String adresse, String type, int nbMaxPrets, int dureeMaxPrets) {
        this.id = id;
        this.nom = nom;
        this.prenom = prenom;
    ...
    public void modifierCetClient(Scanner sc, ArrayList<Historique> historique){
        ...
        do {
            ...
            switch (choix) {
                ...
                case "1":
                    ...
                    historique.add(new Historique(new Date(), "Client", "Modification",getId(), getPrenom()+" "+getNom()+" par "+getPrenom()+" "+nouveauNom));
                    setNom(nouveauNom);
                    break;
                case "2":
                    System.out.println("\t Entrer nouveau prenom");
                    nouveauPrenom = sc.nextLine();
                    historique.add(new Historique(new Date(), "Client", "Modification",getId(), getPrenom()+" "+getNom()+" par "+nouveauPrenom+" "+getNom()));
                    setPrenom(nouveauPrenom);
                    break;
                case "3":
                    System.out.println("\t Entrer nouvelle adresse");
                    nouvelleAdresse = sc.nextLine();
                    historique.add(new Historique(new Date(), "Client", "Modification",getId(), "Adresse de "+getPrenom()+" "+getNom()+" de "+getAdresse()+" a "+nouvelleAdresse));
                    setAdresse(nouvelleAdresse);
                    break;
                case "4":
                    String ancienType = getType();
                    entreeTypeClient(sc);
                    historique.add(new Historique(new Date(), "Client", "Modification",getId(), getPrenom()+" "+getNom()+" de "+ancienType+" a "+getType()));
                    break;
                default: System.out.println("\t Erreur! Taper 0 pour sortire"); break;
            }
        }while (true);
    }

    public void entreeTypeClient(Scanner sc){
        String choixDuType;
        Bibliotheque b = new Bibliotheque();
        do {
            ...
            switch (choixDuType){
                ...
                case "3":
                    ...
                    return;
                default:
                    System.out.println("\t Choix du type Client est non valide veillez réessayer");
                    break;
            }
        }while (true);
    }
